...
/*   Updated: 2024/02/28 03:02:46 by legrandc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_mut(t_vars *vars)
{
	...
	if (pthread_mutex_init(&vars->m_meal, NULL) != 0
		...
		exit_error(MUTEX_ERR, (void *[]){vars->threads, vars->last_meals,
			vars->threads});
	vars->last_meals_cpy = malloc(sizeof(*vars->last_meals_cpy)
			* vars->philo_nb);
	if (!vars->last_meals_cpy)
		exit_error("malloc error", (void *[]){vars->threads, vars->last_meals,
			vars->threads});
